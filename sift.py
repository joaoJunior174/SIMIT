...
sift = cv.xfeatures2d.SIFT_create(300,3,0.004,100,3.0)#300,3,0.004,100,3.0
#sift = cv.xfeatures2d.SIFT_create()#300,3,0.004,100,3.0
...
while(i<=j):
    ...
    if(((index[i][0]-320) < variaveis[1]+20) or ((index[i][0]-320) >  variaveis[3]-20)or ((240-index[i][1]) > variaveis[2]) or ((240-index[i][1]) < variaveis[4])):
        kp.pop(i)
        index.pop(i)
        i=i-1
        j=j-1
